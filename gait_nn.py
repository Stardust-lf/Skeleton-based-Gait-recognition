    ...
    def residual_block(net, ch = 256, ch_inner = 128, scope = None, reuse = None, stride = 1):
        ...
        with slim.arg_scope([slim.convolution2d],
            ...
            with tf.variable_scope(scope, 'ResidualBlock', reuse = reuse):
                ...
                if stride > 1:
                    net = slim.convolution2d(net, ch, kernel_size = 1, stride = stride)

                in_net = slim.batch_norm(in_net)
                in_net = tf.nn.relu(in_net)
                in_net = slim.convolution2d(in_net, ch_inner, 1)

                in_net = slim.batch_norm(in_net)
                in_net = tf.nn.relu(in_net)
                in_net = slim.convolution2d(in_net, ch_inner, 3, stride = stride)

                in_net = slim.batch_norm(in_net)
                in_net = tf.nn.relu(in_net)
                in_net = slim.convolution2d(in_net, ch, 1, activation_fn = None)
            ...
            with slim.arg_scope(self.get_arg_scope(is_training)):
                with tf.variable_scope('DownSampling'):
                    with tf.variable_scope('17x17'):
                        net = slim.convolution2d(net, num_outputs = 256, kernel_size = 1)
                    ...
                    with tf.variable_scope('4x4'):
                        ...
                        net = slim.convolution2d(net, num_outputs = 256, kernel_size = 1)
                        net = slim.convolution2d(net, num_outputs = 256, kernel_size = 3)

                with tf.variable_scope('FullyConnected'):
                    # net = tf.reduce_mean(net, [1, 2], name = 'GlobalPool')
                    net = slim.flatten(net)
                    net = slim.fully_connected(net, 512, activation_fn = None, normalizer_fn = None)

                with tf.variable_scope('Recurrent', initializer = slim.xavier_initializer()):
                    cell_type = {
                ...
                if is_training:
                    ...
                    net = slim.dropout(net, 0.7)

                    with tf.variable_scope('Logits'):
                        net = slim.fully_connected(net, self.num_of_persons, activation_fn = None,
                                                     normalizer_fn = None)
