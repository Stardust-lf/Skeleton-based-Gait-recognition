    ...
    def get_network(self, input_tensor, is_training):
        ...
        with tf.variable_scope('NewInceptionResnetV2'):
            with tf.variable_scope('AuxiliaryScoring'):
                with slim.arg_scope([slim.convolution2d, slim.convolution2d_transpose],
                    ...
                    net = slim.convolution2d(net, num_outputs = self.FEATURES, kernel_size = 1, stride = 1,
                                               scope = 'Scoring_layer')
                    feature = net
                    tf.summary.histogram('Scoring_layer/activations', net, [KEY_SUMMARIES])

                    # Upsampling
                    net = slim.convolution2d_transpose(net, num_outputs = 16, kernel_size = 17, stride = 17,
